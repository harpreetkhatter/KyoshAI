...
export default function ResumePreview({ content }: { content: string }) {
  ...
  if (!content) {
    return (
      <div ref={containerRef} className="w-full overflow-hidden">
        <div className="bg-white shadow-[0_0_60px_-15px_rgba(var(--color-primary),0.15)] border border-white/10" style={{ ...paperStyle, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div ref={containerRef} className="w-full overflow-hidden" style={{ height: `${1123 * scale}px`, maxHeight: '100%' }}>
      <div className="bg-white text-gray-900 shadow-[0_0_60px_-15px_rgba(var(--color-primary),0.15)] border border-white/10" style={paperStyle}>
        <style>{`
