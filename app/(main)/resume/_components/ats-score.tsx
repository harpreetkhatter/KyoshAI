...
export default function ATSScore({ content }: { content: string }) {
    ...
    const isReady = content && content.trim().length >= 50;

    return (
        <div className="gold-border-gradient rounded-lg md:rounded-xl shadow-xl p-2 md:p-4 flex items-center gap-2 md:gap-4 bg-black/80 backdrop-blur">
            <div className="relative w-9 h-9 md:w-12 md:h-12 shrink-0">
                <svg className="w-full h-full" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" r="40" fill="transparent" strokeWidth="8" className="stroke-white/10" />
            ...
            <div className="hidden md:block">
                ...
                <span className={`text-sm font-semibold ${getScoreColor()}`}>{isLoading ? "Analyzing..." : score !== null ? label : (!isReady ? "Add details..." : "Not scored")}</span>
            </div>
            <div className="h-6 md:h-8 w-px bg-border" />
            <button onClick={analyse} disabled={isLoading || !isReady} className="text-[10px] md:text-xs font-semibold text-primary hover:text-primary/80 disabled:opacity-50 transition-colors whitespace-nowrap">
                {isLoading ? "..." : "Analyse"}
            </button>
        </div>
    );
