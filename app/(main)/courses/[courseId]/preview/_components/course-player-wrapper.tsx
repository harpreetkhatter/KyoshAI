...
export default function CoursePlayerWrapper({ slides, chapters }: { slides: any[]; chapters: { chapterId: string; title: string; slides: any[] }[] }) {
    ...
    return (
        <div className="w-full flex flex-col lg:flex-row gap-10 items-start">
            {/* Video Player */}
            <div className="relative group flex-1 w-full lg:w-2/3">
                <div className="relative w-full aspect-video bg-[#050505] rounded-2xl border border-white/10 overflow-hidden shadow-[0_20px_50px_-12px_rgba(0,0,0,0.7)] video-player-container">
                    <Player
                        ref={playerRef}
                    ...
                    <div className="playback-speed-controls">
                        {[1, 1.5, 2].map((speed) => (
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${playbackSpeed === speed
            ...
            {/* Chapter Navigator */}
            {chapters.length > 0 && (
                <div className="w-full lg:w-1/3 flex-[0_0_100%] lg:flex-[0_0_33.333%] space-y-4 sticky top-24">
                    <h2 className="text-xl font-bold text-white/90 tracking-tight">Chapters & Slides</h2>
                    <div className="flex flex-col gap-4 max-h-[70vh] overflow-y-auto pr-2" style={{ scrollbarWidth: 'thin' }}>
                        {chapters.map((ch, i) => (
                            <div
                                key={ch.chapterId}
