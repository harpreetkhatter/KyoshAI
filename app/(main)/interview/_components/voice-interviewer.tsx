    ...
    return (
        <div className="flex flex-col items-center justify-between min-h-[70vh] w-full max-w-4xl mx-auto relative overflow-hidden pb-12">
            ...
            <div className={`relative flex items-center justify-center transition-all duration-700 w-full z-10 ${isCalling ? 'mt-12 mb-8' : 'my-20'}`}>
                <div className={`relative flex items-center justify-center rounded-3xl transition-all duration-700 ${isCalling
                    ? 'h-40 w-full max-w-sm bg-black/40 backdrop-blur-xl border border-primary/40 shadow-[0_20px_50px_-15px_var(--color-primary)] scale-100 z-10'
                    : 'h-40 w-full max-w-xs bg-[#111] border border-white/10 scale-95 opacity-80'
                    }`}>

                    {isCalling && isAgentSpeaking && (
                        <>
                            {/* Inner sound pulse matching the box borders */}
                            <div className="absolute inset-[-15px] rounded-3xl border border-primary/30 animate-pulse duration-[1500ms]" />
                            <div className="absolute inset-[-2px] rounded-3xl ring-2 ring-primary/50 animate-pulse duration-700" />
                        </>
                    )}
